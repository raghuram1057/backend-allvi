...
const appointmentController = {
    ...
    requestAppointment: async (req, res) => {
        try {
            ...
            // 🚀 1. Fetch the user's email and name directly from the profiles table
            ...
            const userName = `${profile.first_name || ''} ${profile.last_name || ''}`.trim() || 'Patient';

            // 🚀 2. Send email using the user's actual email address in the 'from' field
            const data = await resend.emails.send({
                from: `${userName} <${userEmail}>`, 
                to: 'nramraghu143@gmail.com',
